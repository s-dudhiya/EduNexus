...
export function AppSidebar({
  ...
  return (
    <Sidebar
      ...
      <SidebarHeader className="h-16 flex items-center justify-between border-b border-blue-100 bg-white/80 backdrop-blur-sm px-4 py-4">
        <div className={`flex items-center space-x-3 ${isCollapsed ? 'w-full justify-center -ml-0' : '-ml-2'}`}>
                ...
                return (
                  <SidebarMenuItem key={module.id}>
                    <SidebarMenuButton
                      ...
                      isActive={isActive}
                      className={`group relative flex items-center transition-all duration-300 py-2 px-3
                    ...
                    >
                      ...
                      {!isCollapsed && <span className="ml-3">{module.title}</span>}
      ...
      <SidebarFooter className="border-t border-blue-100 bg-white/80 backdrop-blur-sm">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton asChild disabled={isLocked} className="hover:bg-red-50 rounded-md mx-1 my-1">
              <Link to="/auth" className={`flex items-center text-red-600 font-medium hover:text-red-700 transition-colors py-2 px-3 ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
